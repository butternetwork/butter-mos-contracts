...
async function create(salt, bytecode, param) {
  ...
  let salt_hash = await ethers.utils.keccak256(
    await ethers.utils.toUtf8Bytes(salt)
  );
  ...
  let redeploy = false;
  if (code === "0x") {
    let create_code = ethers.utils.solidityPack(
      ["bytes", "bytes"],
      [bytecode, param]
    );
    ...
    console.log(
      "already deploy, please change the salt if if want to deploy another contract ..."
    );
  }
