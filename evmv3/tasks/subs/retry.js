...
task("relay:retry", "retry")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (r && r.logs) {
    r.logs.forEach((log) => {
        if (log.address.toLowerCase() === bridge.address.toLowerCase() && log.topics[0].toLowerCase() === messageInTopic) {
          retryLog = log;
        }
    });
    }
    ...
    let d = ethers.utils.defaultAbiCoder.decode(
      ["uint8", "uint256", "bytes", "bytes", "bytes"],
      payload
    );
    let swapData = d[4]
    await(await bridge.retryMessageIn(chainAndGasLimit, orderId, token, amount, from, payload, swapData)).wait()
  });